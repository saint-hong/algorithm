...
- 정렬 과정 
    - 이미 정렬되어 있는 데이터의 그룹 혹은 묶음들을 하나로 합치는 과정을 통해 정렬을 완료한다.
    - 데이터를 절반으로 나눈다.
    - 각 부분으로 다시 절반으로 나눈다. 이 과정을 재귀호출을 통해 반복하여, 각 부분이 1개의 데이터를 이룰 때까지 반복한다. 이렇게 나누어진 각 부분을 run 이라고 한다.

![mergesortA](./images/mergesortA.png)    


    - 각각의 run 을 다시 합치는데(merge), 이때 정렬을 하면서 합쳐진다. 
    - 1개 -> 2개 -> 4개 -> 8개 의 run 들이 각각 합쳐진다.
    - 마지막 2개의 run 이 남으면, 각 run 은 이미 merge 를 반복하면서 정렬이 된 상태이다. 
    - 왼쪽 run과 오른쪽 run 의 첫 번째 데이터끼리 크기를 비교하여, 작은 값을 새로운 리스트에 저장하고, 해당 run 에서는 제거한다.
    - 왼쪽, 오른쪽 run 의 데이터가 더 이상 비교를 할 수 없을 때까지 반복한다.
    - 작은 값 순서로 저장 된 새로운 리스트를 반환한다.
    
```
1 부터 2의 배수로 하나의 run 에 들어가는 데이터 수를 늘려서 병합하는 과정을 반복한 후,
하나의 run 의 크기가 원래 데이터의 절반이 되었을 때, 2개의 run 의 각 데이터를 비교하여 작은 수 순서로 새로운 리스트에 저장하고 반환한다.
```
